...
const ProductCard = ({ product }) => {
  ...
  return (
    <div
      className="relative flex flex-col border bg-white shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden group"
      style={{ minWidth: 0 }}
    >
      {/* Clickable Product Link - Image Section */}
      <Link 
        to={`/products/${product.id}`}
        className="w-full p-4 box-border block hover:opacity-90 transition-opacity duration-200"
      >
        <div
